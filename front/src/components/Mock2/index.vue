  ...
  <div>
    ...
    <div v-show="isShow" class="mock-content">
      ...
      >
        ...
        <el-form-item :label="$t('Mock.ipFilter')" prop="ip">
          <el-input
            v-model="formData.ip"
            :disabled="formData.isFilterIp !== 1"
            :placeholder="$t('Mock.ipFilterTip')"
            maxlength="255"
            show-word-limit
          >
            <template slot="prepend">
              <el-switch
                v-model="formData.isFilterIp"
                active-text=""
                inactive-text=""
                :active-value="1"
                :inactive-value="0"
  ...
  methods: {
    ...
    onMockAdd() {
      ...
      this.getNextVersion(this.item.id, nextVersion => {
        Object.assign(node, {
          id: '' + this.nextId(),
          name: `${this.item.name}-${nextVersion}`,
          path: this.getPath(),
          httpMethod: this.item.httpMethod,
          version: nextVersion,
    ...
    onMockDelete(data) {
      if (data.isNew) {
        ...
        this.post('/doc/mock2/delete', data, resp => {
    ...
    onSave() {
      this.validate(() => {
        ...
        this.formData.httpMethod = this.item.httpMethod
        this.formData.path = this.getPath()
        this.post('/doc/mock2/save', this.formData, resp => {
